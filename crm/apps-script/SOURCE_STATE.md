...
> The same publication includes the physical-stock fix: only `Нове`, `В обробці`,
> and `Передзамовлення` restore a quantity into `Фізично`; historical
> `Відправлено`, `Отримано`, and payment-only rows do not. The owner screenshot
> confirms Outlet Mix now shows `Фізично 0` and `Очікується 80`. Live QA of the
> timeout/capacity paths remains required. Direct `git diff --no-index` against
> the complete V156 export confirms the deployed local source delta is 205
> insertions / 37 deletions; no assumed intermediate source remains in the handoff.
...
> not an independent V154 source comparison or live behavior check. The subsequent plain-language
> dashboard hint changes do not modify `Code.gs`.
