...
class Order extends \Opencart\System\Engine\Model {
	...
	public function addHistory(int $order_id, int $order_status_id, string $comment = '', bool $notify = false, bool $override = false): void {
		...
		if ($order_info) {
			...
			$booster_crm_history_query = $this->db->query("SELECT COUNT(*) AS `total` FROM `" . DB_PREFIX . "order_history` WHERE `order_id` = '" . (int)$order_id . "'");
			$booster_crm_event = ((int)($booster_crm_history_query->row['total'] ?? 0) === 0) ? 'order_add' : 'order_status';

			$this->db->query("INSERT INTO `" . DB_PREFIX . "order_history` SET `order_id` = '" . (int)$order_id . "', `order_status_id` = '" . (int)$order_status_id . "', `notify` = '" . (int)$notify . "', `comment` = '" . $this->db->escape($comment) . "', `date_added` = NOW()");
			$this->load->controller('extension/telegram_notify/event/telegram.sendOrderNotification', (int)$order_id, (int)$order_status_id);
			// CHECKOUT-002_ASYNC_ORDER_SIDE_EFFECTS_20260719: direct Telegram call is the single queued delivery path.

			$this->boosterCrmSync($order_id, $booster_crm_event);
        $this->ncrmOrderSync($order_id, $booster_crm_event);
	...
	/**
	 * Sync order snapshot to Booster CRM without blocking checkout/admin status updates.
	 */
...
private function boosterCrmSync(int $order_id, string $event): void {
		try {
			register_shutdown_function(function () use ($order_id, $event): void {
				...
				try {
					$file = DIR_SYSTEM . 'library/booster_crm_sync.php';

					if (is_file($file)) {
						require_once($file);

						$sync = new \Opencart\System\Library\BoosterCrmSync($this->registry);
						$sync->syncOrder($order_id, $event);
					}
				} catch (\Throwable $e) {
					$this->log->write('Booster CRM sync failed for order ' . $order_id . ': ' . $e->getMessage());
				}
			});
		} catch (\Throwable $e) {
			$this->log->write('Booster CRM sync failed for order ' . $order_id . ': ' . $e->getMessage());
