...
<div class="form-check form-switch mb-3"><input class="form-check-input" type="hidden" name="payment_pumb_credit_public" value="0"><input class="form-check-input" type="checkbox" name="payment_pumb_credit_public" value="1"{% if payment_pumb_credit_public %} checked{% endif %}><label class="form-check-label">Показувати ПУМБ усім клієнтам</label></div>
...
try {
    writeFileChecked($root.'/'.$rel[0],$catalog); writeFileChecked($root.'/'.$rel[1],$model); writeFileChecked($root.'/'.$rel[2],$admin); writeFileChecked($root.'/'.$rel[3],$twig);
    lint($root.'/'.$rel[0]); lint($root.'/'.$rel[1]); lint($root.'/'.$rel[2]);
    ...
    if (!$rowsToken) { $key='payment_pumb_credit_preview_token'; $value=''; $stmt->bind_param('sss',$code,$key,$value); need($stmt->execute(),'cannot insert preview token setting'); }
    if (!$rowsPublic) { $key='payment_pumb_credit_public'; $value='0'; $stmt->bind_param('sss',$code,$key,$value); need($stmt->execute(),'cannot insert public setting'); }
    $stmt->close(); writeFileChecked($marker,'PAY-002 WP1 applied ' . date('c') . PHP_EOL); out('cwd='.$root); out('backup='.$backup); out('changed='.implode(',', $rel)); out('payment_pumb_credit_status_preserved=yes'); out('done=ok');
} catch (Throwable $e) {
    foreach($rel as $f) @copy($backup.'/'.$f,$root.'/'.$f);
    $db->query("DELETE FROM `" . DB_PREFIX . "setting` WHERE store_id=0 AND code='payment_pumb_credit' AND `key` IN ('payment_pumb_credit_preview_token','payment_pumb_credit_public')");
