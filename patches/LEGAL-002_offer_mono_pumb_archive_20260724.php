...
const PATCH_NAME = 'LEGAL-002_offer_mono_pumb_archive_20260724';
const LANGUAGE_ID = 4;
...
function bs_one_by_title(mysqli $db, string $table, string $title): ?array {
    ...
    $lang = LANGUAGE_ID; $stmt->bind_param('is', $lang, $title); $stmt->execute(); $r = $stmt->get_result();
    $rows = $r->fetch_all(MYSQLI_ASSOC); $r->free(); $stmt->close();
...
function bs_route(mysqli $db, array $route): ?array {
    ...
    $slug = ARCHIVE_SLUG; $stmt->bind_param('s', $slug); $stmt->execute(); $r = $stmt->get_result(); $rows = $r->fetch_all(MYSQLI_ASSOC); $r->free(); $stmt->close();
...
function bs_insert_archive_description(mysqli $db, string $table, int $id, string $html): void {
    $stmt = $db->prepare('INSERT INTO `' . $table . '` (information_id, language_id, title, description, meta_title, meta_description, meta_keyword) VALUES (?, ?, ?, ?, ?, ?, ?)');
    $lang = LANGUAGE_ID; $title = ARCHIVE_TITLE; $metaTitle = ARCHIVE_TITLE; $empty = '';
    $stmt->bind_param('iisssss', $id, $lang, $title, $html, $metaTitle, $empty, $empty); $stmt->execute(); $stmt->close();
}
...
function bs_run(): void {
    ...
    $db = bs_connect();
    try {
        $prefix = (string) DB_PREFIX; $info = bs_table($prefix, 'information'); $desc = bs_table($prefix, 'information_description');
        foreach ([$info, $desc] as $table) if (!bs_table_exists($db, $table)) bs_fail('Required table not found: ' . $table);
        bs_require_columns(bs_columns($db, $info), ['information_id','bottom','sort_order','status'], $info);
        bs_require_columns(bs_columns($db, $desc), ['information_id','language_id','title','description','meta_title','meta_description','meta_keyword'], $desc);
        ...
        $archiveOk = $archive !== null && hash('sha256', (string)$archive['description']) === ARCHIVE_SHA256;
        $routeOk = $archive !== null && bs_route_is_correct($routeRow, $route, (int)$archive['information_id']);
        if ($offerOk && $archiveOk && $routeOk) { bs_log('already_applied', 'yes'); bs_log('done', 'ok'); @unlink(__FILE__); bs_log('self_delete', file_exists(__FILE__) ? 'failed' : 'ok'); return; }
        ...
        bs_json_backup($backupDir, 'archive_before', ['information_table' => $info, 'description_table' => $desc, 'archive_row' => $archive, 'seo_table' => $route['table'], 'seo_route' => $routeRow]);
        ...
        try {
            ...
            if ($archive === null) { $db->query('INSERT INTO `' . $info . '` (bottom, sort_order, status) VALUES (0, 0, 1)'); $archiveId = (int)$db->insert_id; bs_insert_archive_description($db, $desc, $archiveId, $archiveHtml); bs_insert_route($db, $route, $archiveId); bs_log('created_archive_information_id', (string)$archiveId); }
