...
if (text.indexOf('/orders') === 0) { tgCommandOrders_(chatId); return; } if (text.indexOf('/pick_news') === 0) { tgCommandNews_(chatId); return; } if (text.indexOf('/delete_news') === 0) { tgCleanNews_(chatId); return; }
...
try {
if (data === 'main_menu') { tgAnswerCallback_(callbackId, ''); tgShowMainMenu_(chatId, messageId); return; } if (data === 'orders_list' || data === 'back_orders') { tgAnswerCallback_(callbackId, ''); tgCommandOrders_(chatId, messageId); return; } if (data === 'news_list') { tgAnswerCallback_(callbackId, ''); tgCommandNews_(chatId, messageId); return; } if (data.indexOf('news_pick_') === 0) { tgAnswerCallback_(callbackId, ''); tgShowNewsPost_(chatId, data.substring('news_pick_'.length)); return; } if (data === 'news_clean') { tgAnswerCallback_(callbackId, 'Чищу...'); tgCleanNews_(chatId, messageId); return; } if (data.indexOf('news_done_') === 0) { const newsId = data.substring('news_done_'.length); const newsCandidate = crmGetNewsCandidates_(36500).filter(function(item) { return item.id === newsId; })[0]; if (!newsCandidate) { tgAnswerCallback_(callbackId, 'Не знайдено'); return; } _getCrmSs().getSheetByName('Новини_кандидати').getRange(newsCandidate.rowIndex, 10).setValue('posted'); tgAnswerCallback_(callbackId, 'Позначено'); tgShowMainMenu_(chatId, messageId); return; }
...
const keyboard = [[{ text: 'Активні замовлення', callback_data: 'orders_list' }], [{ text: 'Новини', callback_data: 'news_list' }], [{ text: 'Очистити новини (>3д)', callback_data: 'news_clean' }]];
if (messageId) tgEditMessage_(chatId, messageId, text, keyboard); else tgSendMessage_(chatId, text, keyboard);
}
...
function setupNewsSheet() { return setupNewsSheet_(); }
...
function seedNewsTest() { return seedNewsTestRows_(); }
function seedNewsTestRows_() {
  const ss = _getCrmSs();
  const sheet = ss.getSheetByName('Новини_кандидати');
  if (!sheet) throw new Error('Спочатку запусти setupNewsSheet');
  const now = new Date();
  const old = new Date(Date.now() - 4 * 86400000); // 4 дні тому
  const rows = [
    ['NEWS-T1', now, 'Pokémon',  'Тест: свіжий пост 1', 'Тестовий текст поста 1. Цей текст можна копіювати довгим тапом.', 'https://example.com/article1', 'https://example.com/img1.jpg', '', '', 'new', 'guid-t1'],
    ['NEWS-T2', now, 'One Piece', 'Тест: свіжий пост 2', 'Тестовий текст поста 2.', 'https://example.com/article2', 'https://example.com/img2a.jpg', 'https://example.com/img2b.jpg', '', 'new', 'guid-t2'],
    ['NEWS-T3', old, 'MTG',       'Тест: старий пост (4 дні)', 'Тестовий текст поста 3.', 'https://example.com/article3', 'https://example.com/img3.jpg', '', '', 'new', 'guid-t3']
  ];
