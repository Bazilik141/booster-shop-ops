...
if (!purchases) return [];
const saleSort = dateSortValue_(saleDate);
const allowed = { 'На складі': true, 'Частково продано': true, 'Продано': true };
...
const purchases = ss.getSheetByName('Закупки');
if (!purchases) throw new Error('Не знайдено вкладку Закупки.');
const updatable = { 'На складі': true, 'Частково продано': true };
const allowedForFifo = { 'На складі': true, 'Частково продано': true, 'Продано': true };
const lastRow = Math.max(purchases.getLastRow(), 3);
