...
/**
 * Fix Booster CRM sync replay after queued payload token drift.
 * - Keeps/fixes live sync redirect handling and strict JSON ok:true validation.
 * - Deletes queued payloads older than 21 days.
 * - Filters owner/test orders and TEST SKU before CRM sync.
 * - Re-signs queued payloads with current SECRET_TOKEN from booster_crm_sync.php before replay.
 */

$target = __DIR__ . '/system/library/booster_crm_sync.php';
$backupDir = __DIR__ . '/_boostershop_patch_backups/booster_crm_sync_20260616_resign';
$retentionDays = 21;
$log = [];
...
function patch_fail(array $log, string $message): void {
    ...
    if (defined('DIR_STORAGE')) {
        ...
        if (is_dir($logDir) || @mkdir($logDir, 0755, true)) {
            @file_put_contents($logDir . DIRECTORY_SEPARATOR . 'booster_crm_sync_patch_20260616.log', implode(PHP_EOL, $log) . PHP_EOL, FILE_APPEND | LOCK_EX);
        }
    }
    exit(1);
}
...
function payload_is_test_order(array $payload): bool {
    ...
    if ($name !== '' && in_array($name, ['євгеній леусенко', 'евгений леусенко', 'evgenii leusenko', 'yevhenii leusenko', 'yevgeniy leusenko'], true)) {
        return true;
    }
    ...
    foreach (($payload['products'] ?? []) as $product) {
        ...
        foreach (['test sku', 'test-sku', 'testsku', 'тестова позиція'] as $marker) {
...
function replay_queue(string $url, string $secretToken, int $retentionDays, array &$log): array {
    ...
    foreach (glob($queueDir . DIRECTORY_SEPARATOR . '*.json') ?: [] as $file) {
        ...
        if (!isset($groups[$orderKey])) {
            $groups[$orderKey] = ['latest' => $file, 'payload' => $payload, 'ts' => $ts, 'files' => [$file]];
        } else {
            $groups[$orderKey]['files'][] = $file;
            if ($ts > $groups[$orderKey]['ts']) {
                $groups[$orderKey]['latest'] = $file;
                $groups[$orderKey]['payload'] = $payload;
                $groups[$orderKey]['ts'] = $ts;
            }
        }
    }
...
if (!preg_match("/private const SECRET_TOKEN = '([^']+)'/", $source, $tokenMatch)) {
    patch_fail($log, 'cannot read SECRET_TOKEN');
}
$url = $urlMatch[1];
$secretToken = $tokenMatch[1];
if ($url === '' || $url === 'PASTE_GOOGLE_APPS_SCRIPT_WEB_APP_URL_HERE' || $secretToken === '' || $secretToken === 'CHANGE_ME') {
    patch_fail($log, 'CRM URL or token is not configured');
}

$updated = $source;
$changed = false;
if (strpos($updated, "\tprivate const TIMEOUT_SECONDS = 2;\n\tprivate const CONNECT_TIMEOUT_MS = 700;\n\tprivate const TIMEOUT_MS = 1800;") !== false) {
...
if (strpos($updated, $postredirBlock) !== false) {
    $updated = str_replace($postredirBlock, "\n", $updated);
    $changed = true;
}
...
if ($changed) {
    ...
    patch_log($log, 'library_change=not_needed');
}

try {
    $stats = replay_queue($url, $secretToken, $retentionDays, $log);
