...
internal sealed class InstallerEngine
{
    ...
    private const string LegacyTranslatorPluginRelative = "BepInEx/plugins/Translator";
    private const string LegacyTranslatorDllRelative = "BepInEx/plugins/Translator/Translator.dll";
    private const string LegacyTranslatorConfigRelative = "BepInEx/config/shaklin.Translator.cfg";
    private const string LegacyArchiveRootRelative = "TCGCardShopSimulatorUA-backups";
    ...
    public InstallPlan BuildPlan(string gameRoot, PayloadExtractor payload)
    {
        ...
        if (HasConflictingLegacyTranslator(gameRoot))
        {
            plan.Warnings.Add("Знайдено старий Translator.dll. Він завантажується після цієї локалізації та перезаписує частину українських рядків англійськими.");
            plan.Actions.Add(new(PlanActionKind.ArchiveConflictingLegacyPlugin, LegacyTranslatorPluginRelative,
                "Перемістити старий Translator у папку резервної копії поза BepInEx; файли не видаляються."));
        }
    ...
    public void InstallOrUpdate(string gameRoot, PayloadExtractor payload, string modVersion, Action<string>? log = null)
    {
        ...
        var mutationPaths = plan.Actions
            .Where(x => x.Kind != PlanActionKind.SkipSame &&
                        (payloadFiles.ContainsKey(x.RelativePath) || oldByPath.ContainsKey(x.RelativePath)))
        ...
        LegacyTranslatorArchive? legacyArchive = null;

        try
        {
            legacyArchive = ArchiveConflictingLegacyTranslator(gameRoot, stamp, log);
        ...
        catch (Exception ex)
        {
            ...
            {
                try { RestoreLegacyTranslatorArchive(legacyArchive, log); }
                catch (Exception restoreEx) { rollbackErrors.Add($"legacy Translator: {restoreEx.Message}"); }
            }
    ...
    public List<string> Uninstall(string gameRoot, Action<string>? log = null)
    {
        ...
        try
        {
            ...
            {
                if (IsSharedRuntimePath(file.RelativePath))
                {
                    messages.Add($"Залишено спільний BepInEx/Doorstop runtime: {file.RelativePath}");
                    continue;
                }
    ...
    private static bool HasConflictingLegacyTranslator(string gameRoot)
        => File.Exists(PathSafety.SafeCombine(gameRoot, LegacyTranslatorDllRelative));

    private static LegacyTranslatorArchive? ArchiveConflictingLegacyTranslator(string gameRoot, string stamp, Action<string>? log)
    {
        if (!HasConflictingLegacyTranslator(gameRoot)) return null;

        var sourcePluginDir = PathSafety.SafeCombine(gameRoot, LegacyTranslatorPluginRelative);
        var sourceConfig = PathSafety.SafeCombine(gameRoot, LegacyTranslatorConfigRelative);
        var archiveRoot = PathSafety.SafeCombine(gameRoot, $"{LegacyArchiveRootRelative}/legacy-Translator-{stamp}");
        var archivedPluginDir = Path.Combine(archiveRoot, "BepInEx", "plugins", "Translator");
        var archivedConfig = Path.Combine(archiveRoot, "BepInEx", "config", "shaklin.Translator.cfg");

        Directory.CreateDirectory(Path.GetDirectoryName(archivedPluginDir)!);
        Directory.Move(sourcePluginDir, archivedPluginDir);
        try
        {
            if (File.Exists(sourceConfig))
            {
                Directory.CreateDirectory(Path.GetDirectoryName(archivedConfig)!);
                File.Move(sourceConfig, archivedConfig);
            }
        }
        catch
        {
            Directory.Move(archivedPluginDir, sourcePluginDir);
            throw;
        }

        log?.Invoke($"Старий Translator перенесено до резервної копії: {Path.GetRelativePath(gameRoot, archiveRoot)}");
        return new LegacyTranslatorArchive(sourcePluginDir, archivedPluginDir,
            File.Exists(archivedConfig) ? sourceConfig : null,
            File.Exists(archivedConfig) ? archivedConfig : null);
    }

    private static void RestoreLegacyTranslatorArchive(LegacyTranslatorArchive archive, Action<string>? log)
    {
        if (archive.ArchivedConfig is not null && archive.SourceConfig is not null && File.Exists(archive.ArchivedConfig) && !File.Exists(archive.SourceConfig))
        {
            Directory.CreateDirectory(Path.GetDirectoryName(archive.SourceConfig)!);
            File.Move(archive.ArchivedConfig, archive.SourceConfig);
        }
        if (Directory.Exists(archive.ArchivedPluginDirectory) && !Directory.Exists(archive.SourcePluginDirectory))
        {
            Directory.CreateDirectory(Path.GetDirectoryName(archive.SourcePluginDirectory)!);
            Directory.Move(archive.ArchivedPluginDirectory, archive.SourcePluginDirectory);
        }
        log?.Invoke("Після помилки відновлено старий Translator із резервної копії.");
    }

    private sealed record LegacyTranslatorArchive(
        string SourcePluginDirectory,
        string ArchivedPluginDirectory,
        string? SourceConfig,
        string? ArchivedConfig);
