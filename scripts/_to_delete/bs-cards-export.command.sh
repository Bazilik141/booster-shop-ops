...
if ($lang === []) fail('no rows in product_description');
...
if (!@mkdir($dir . '/products', 0700, true) && !is_dir($dir . '/products')) fail('cannot create ' . $dir);
if (!@mkdir($dir . '/categories', 0700, true) && !is_dir($dir . '/categories')) fail('cannot create categories dir');
...
cd ~/public_html \
&& OUT=$(php ~/bs-cards-export.php) && echo "$OUT" \
&& DIR=$(echo "$OUT" | sed -n 's/^dir: //p') \
&& tar -czf "${DIR}.tar.gz" -C "$(dirname "$DIR")" "$(basename "$DIR")" \
&& rm -rf "$DIR" ~/bs-cards-export.php \
&& echo "---" && ls -lh "${DIR}.tar.gz"
