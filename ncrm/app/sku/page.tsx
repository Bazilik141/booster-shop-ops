  ...
  return (
    <main className="page">
      ...
      <section className="card table-card"><div className="table-wrap"><table><thead><tr><th>SKU</th><th>Назва</th><th>РРЦ</th><th>Склад</th><th>Продано 30 днів</th><th>Виручка 30 днів</th><th>FIFO mgmt</th><th>Сигнал</th></tr></thead><tbody>
        {catalogue.rows.map((row) => <tr key={row.productId}><td>{row.sku}</td><td>{row.fullName ?? row.name ?? "—"}</td><td>{money(row.currentRrc)}</td><td>{row.stockQty ?? "—"}</td><td>{row.units30d}</td><td>{money(row.revenue30d)}</td><td>{money(row.warehouseMgmtCost)}</td><td>{row.alert ?? "—"}</td></tr>)}
