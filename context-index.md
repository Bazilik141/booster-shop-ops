...
| TECH-015 WP3 | GA4 — решта воронки: add_payment_info / add_shipping_info, add_to_cart з плиток каталогу, remove_from_cart у міні-кошику; починається з read-only звірки по свіжому бекапу | handoffs/handoff_TECH-015_ga4-funnel-remainder_wp3_20260907.md | diagnostics/TECH-015_ga4-purchase_wp1_review_20260907.md · diagnostics/TECH-015_ga4-begin-checkout_wp2_review_20260907.md |
...
| PAY-002-CONT | Точка входу для продовження роботи над ПУМБ у новій сесії — стан справ, доведені факти, що лишилось | **Читати першим і замість історії:** `handoffs/handoff_PAY-002_session-continuation_20260826.md` (§10 — оновлення 2026-08-28: 7-денне вікно і термін 5 закриті банком; гейт по токену замість групи) → `handoffs/handoff_PAY-002_pumb-checkout-card-token-gate_20260828.md` (ТЗ на картку ПУМБ у чекауті, WP1+WP2) |
...
| CRM-011 | Аналітичний шар дашборда: Фінанси (P&L + рух коштів), пошук/фільтри й клієнт у замовленнях, режими та сегментація клієнтів, повільний залишок, фільтри роадмапу | **Читати:** `handoffs/handoff_CRM-011_dashboard-finance-crm-upgrade_20260908.md`. Заведено 08.09. Шість окремо-деплойних пакетів: WP1 повільний залишок (лише фронт), WP2 бекенд одним проходом (`Code.gs`: дати оплати/створення, `finance_report`, збагачення `orders`, `ltv_report`, `client_orders`, константи сегментації, fallback курсу 3.5→3.2), WP3 вкладка Фінанси, WP4 Замовлення, WP5 Клієнти, WP6 фільтри роадмапу. ⚠ Phase 0 по P&L уже виконаний — `plans/crm-financial-model_2026-06-26.md` §G/§K: `V` вже мінус управлінська собівартість, пакування, комісії й доставку магазину; літери колонок застаріли, звіряти по назві заголовка. ⚠ Дедуплікація операційних витрат уже існує — `Витрати` колонка L. ⚠ Маркетинг НЕ є order-level рядком P&L (3D-P, `Code.gs`:9436). ⚠ Дві різні множини продажів: `_getCrmSalesRows()` (фільтрована, для Фінансів) vs `_getCrmSalesRowEntries()` (сира, для Замовлень) — розбіжність має бути показана в UI. ⚠ WP2 додає колонки в `Закупки` і `Продажі` → `OPS-CRMINTEGRITY` діє повністю, колонки тільки в кінець аркуша, `_getCrmSalesRowEntries()` читає фіксовані 32 колонки. ⚠ Рішення власника 08.09: маска ПІБ/телефону знімається; післяплата = дата замовлення + 3 дні; реальний відтік по товару = поповнення балансу ZenMarket, а не оплата лоту. Дашборд PC-only, деплою не має — Ctrl+F5. || РАУНД 1 (08–09.09, Codex): `diagnostics/CRM-011_dashboard-finance-crm-upgrade_report_20260908.md`. Рев'ю Claude — `diagnostics/CRM-011_claude-review_20260909.md`, вердикт RETURN FOR CHANGES, 8 блокерів. ⚠ Читати амендмент у кінці рев'ю: фіскальний чек, окремий Apps Script проєкт алертів, сторінка «Увага», дати приходу і перебудова Огляду — це НЕ scope creep, власник замовив їх напряму в чаті з Codex, ратифіковано 09.09. ⚠ Головний дефект раунду 1: три функції в `Code.gs` оголошені двічі (`crmGetOrders_` 5869/9873, `apiLtvReport_` 6489/9852, `apiQualifiedClientsReport_` 6518/9862) — нові реалізації дописані в кінець файлу, працює лише завдяки порядку оголошень; у дашборді той самий шаблон (4 мертві клієнтські функції). || РАУНД 2: `handoffs/handoff_CRM-011-R2_defect-fixes_20260909.md`, два проходи — Pass A лише фронт без публікації (сортування замовлень, маржа<15%, 6 пресетів, вікно порівняння, delta, чистка мертвого коду, розвантаження таблиці клієнтів), Pass B один реліз Apps Script (дедуп функцій, повний P&L, New vs Repeat як таблиця, avg_order, distribution сегментів). ⚠ Перед Pass B власник робить копію аркуша `Продажі`: раунд 1 залив 108 платежів ZenMarket у живі дані без артефакту відкату |
| CRM-012 | ZenMarket: цілісність балансу, місяць визнання доходу передзамовлень, назви 3D-виробів | **Читати:** `handoffs/handoff_CRM-012_zenmarket-integrity-and-recognition_20260910.md` + вердикт рев'ю `diagnostics/CRM-011_round2-review_20260910.md`. Заведено 10.09 як продовження CRM-011 (CRM-011 НЕ переввідкривається). WP0 закриття гейту дзеркала на V171, WP1 лічильник поповнень без гривні, WP2 ₴22 000 за ZEN-HIST-008 + показ дірки в Cash Flow, WP3 закупки не через ZenMarket, WP4 місяць визнання передзамовлень, WP5 назви 3D-виробів. ⚠ ZenMarket-шар і розширення міграції ВЖЕ ЖИВІ у V171 попри «publication not performed» в обох звітах Codex. ⚠ Дзеркало V171 звірено побайтово; CSV-експорт ламає рядки з подвійними лапками, надалі просити .gs/.txt. ⚠ WP3 корінь: addPurchase() жорстко пише zenmarket_jp і синхронізує з force_zenmarket:true — дашбордний шлях коректний, тобто правило різне за точкою входу; ремонт LOT-0181 / 1158736408 (OLX). ⚠ WP4: зараз передзамовлення входить у місяць створення за Датою продажу, тому закритий місяць змінюється заднім числом; для історії проксі — Дата фіксації собівартості (колонка 32, пишеться, ніде не читається). ⚠ НЕ чіпати: форму поповнення ZenMarket (обидві суми вже факти), курс 3.2, розкладку P&L/Cash Flow поруч, рівний поділ собівартості у container_to_units. Notion: `3d76bf20-bdb4-816d-9e2f-f8eec7058e7c` |
...
`handoffs/handoff_CONTENT-QUALITY_wave_20260828.md` (чинний, 28.08) — єдиний
хендоф хвилі, шість work packages: контент 28 карток, ключі категорії 73,
створення `BR-CHARM-200`, `PKM-JP-SVEL-SET` і семи kit card `FIG-*-300`,
звірка РРЦ з CRM. Замінює `handoff_CONTENT-QUALITY_patch-30-cards_20260827.md`
і три ранери від 27.08 (розбір їхніх дефектів —
`diagnostics/CONTENT-QUALITY_patch-review_20260827.md`).

Вхідні дані хвилі: `BOOSTER-SHOP_CONTENT-QUALITY_RELEASE_20260825_v2.zip` у корені
(з уже застосованим префіксом `/product/`),
`handoffs/addendum_CONTENT-QUALITY_faq-recovery-readycopy_20260828.md`,
`handoffs/handoff_3D-P-CARDCONTENT_kit-cards-7sku_20260828.md`,
`Booster Shop CRM — облік товарів - РРЦ.csv`,
`live-snapshots/20260828_cards-export/`.

⚠ У знімку 28.08 структурні колонки недійсні: експортер парсить опис без
декодування, тому `h2/h3/strong/ul/a` = 0 і `NO_FAQ` стоїть на всіх 94 картках.
Назви, слаги, статуси, категорії, мета й атрибути — правильні.

⚠ Три картки хвилі вже видимі на проді: 145 `YGO-JP-QCAC-BBX`, 151
`YGO-JP-BETB-BBX`, 152 `YGO-JP-BETB-BST`. Решта 25 — `status = 0`.

⚠ `05_CATEGORY_AND_GLOBAL_PATCH.md` §1, §2, §4, §5 описують роботу, яка на проді
вже зроблена (атрибути 43/44, назви категорій 73/74). Тільки перевірка.

Аудити: `diagnostics/CONTENT-QUALITY_release-package-v2-audit_20260827.md`,
`diagnostics/CONTENT-QUALITY_addendum-v2-and-kitcards-qa_20260828.md`.
Передісторія: `handoffs/handoff_CONTENT-QUALITY_session-continuation_20260825.md`.
