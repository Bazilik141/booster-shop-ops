...
## 8. Рішення виконавця — три, усі зафіксовані

### 8.1 FAQ [3] на `YGO-JP-QCAC-BBX` — прибраний непідтверджений токен QCSR

Corrections §4.9 зняли з опису речення про «Secret Rare та Quarter Century
Secret Rare варіанти», бо QCSR не підтвердився при перевірці. Той самий токен
лишався у FAQ [3] тієї ж картки — тобто клієнтський текст і далі припускав
існування варіанта, який ми не змогли підтвердити.

```
було:  Чи гарантується конкретна QCSR або альтернативна ілюстрація?
       Ні. Офіційна сторінка описує доступні варіанти карт, але не гарантує
       конкретну карту або варіант в окремому боксі.

стало: Чи гарантується конкретна карта або рідкість?
       Ні. Офіційна сторінка описує пул сету, але не гарантує конкретну карту
       чи рідкість в окремому боксі.
```

Захисний сенс той самий, непідтвердженого токена немає. Це застосування
правила §4.9 до сусіднього речення, а не нове рішення — але воно поза буквою
corrections, тому винесене сюди для рев'ю. Відкотити тривіально.

`meta_keyword` цієї картки й далі містить `QCSR QCAC` — лишив як у payload:
це пошуковий токен, не клієнтське твердження, і воно не рендериться. Прапорець
підіймаю, рішення за власником.
...
- **CRM-005.** Дев'ять SKU мають існувати в CRM до того, як товари стануть
  видимими.
- **`YGO-JP-QCAC-BBX` і товар 83** — той самий сет (бокс і бустер). Перед
  публікацією звірити факти обох карток між собою; товар 83 патч не чіпав.
